...
using Contracting.Application.Features.Business.FinancialClearance.Command.ReassignFinancialClearance;
...
namespace Contracting.API.Controllers
{
    ...
    public class FinancialClearanceController : APIBaseController
    {
        ...
        public async Task<IActionResult> Reassign(Guid id, [FromBody] ReassignFinancialClearanceDto dto)
        {
            var result = await _mediator.Send(new ReassignFinancialClearanceCommand(id, dto));
            return result.Match(r => Ok(r), errors => Problem(errors));
        }
    }
}
